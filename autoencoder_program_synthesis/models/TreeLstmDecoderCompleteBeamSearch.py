...
class TreeLstmDecoderComplete(nn.Module):
    ...
    def forward(self, z, target=None, idx_to_label=None, nameid_to_placeholderid=None):
        ...
        # We are evaluating and we cannot use training forcing and we generate tree by tree
            ...
            for index in range(z.shape[0]):
                # Reverse nameid to placeholderid to get back the names
                ...
                # Split the latent vector in hidden and cell states
                h_parent, c_parent = torch.split(
                    z[index], int(len(z[index])/2)
                )

                # Set initial parent and sibling states
                ...
                # Set the first node to root node and set the parent state of the root node
                root_node, parent_state = self.init_decode_eval(parent_state, sibling_state, label_to_idx)
                topk_nodes =  self.decode_eval(parent_state, sibling_state, idx_to_label, label_to_idx, placeholderid_to_nameid, root_node)

                beam_nodes = topk_nodes
                end_nodes = []

                while len(beam_nodes) > 0:
                    current_nodes = []
                    for node in beam_nodes:
                        # print(node.token, node.node_type, node.is_parent, node.processed_parent, node.has_sibling, node.processed_sibling)
                        if node.node_type == 'RES' or not self.params['INDIV_LAYERS_VOCABS']:
                            emb_label = self.embedding_layers[node.node_type](
                                torch.tensor([node.token])).view(-1, self.params['EMBEDDING_DIM'])
                        else:
                            emb_label = self.embedding_layers[node.node_type](
                                torch.tensor([node.token])).view(-1, self.params['LEAF_EMBEDDING_DIM'])



                        if node.has_sibling and not node.processed_sibling:
                            node.processed_sibling = True

                            if node.is_parent and not node.processed_parent:
                                node.parent_node.nodes_to_process.append(node)


                            current_nodes.extend(self.eval_step_sibling(emb_label, node, idx_to_label, label_to_idx, placeholderid_to_nameid))

                                
                        elif node.is_parent and not node.processed_parent:
                            node.processed_parent = True
                            current_nodes.extend(self.eval_step_parent(emb_label, node, idx_to_label, label_to_idx, placeholderid_to_nameid))


                        elif len(node.nodes_to_process) > 0:
                            print('-------------------')
                            node_to_process = node.nodes_to_process[-1]
                            node_to_process.parent_node = node.parent_node
                            beam_nodes.append(node_to_process)
                            # node.nodes_to_process.remove(node_to_process)

                        else:
                            end_nodes.append(node.node)


                    current_nodes.sort(key=lambda node: node.logprob, reverse=True)
                    print(current_nodes)
                    beam_nodes = current_nodes[:self.params['BEAM_WIDTH']]
                    # if len(current_nodes) > 0:
                    #     print(current_nodes[0].nodes_to_process)
                ...
                # parent_state, sibling_state, parent_node = self.update_states(predicted_label, node_type, parent_state,
                #                                             sibling_state, is_parent, has_sibling, node,
                #                                             parent_state_updated, parent_node, idx_to_label, label_to_idx, placeholderid_to_nameid)
                trees.append(end_nodes)
                break
    ...
    def eval_step_parent(self, emb_label, node, idx_to_label, label_to_idx, placeholderid_to_nameid):
        ...
        node.parent_state[-1] = node.parent_state_updated
        # update parent state and parent_node of tree
        for i in range(self.params['NUM_LSTM_LAYERS']):
                if i == 0:
                    node.parent_state[i] = self.lstms_parent[i](emb_label, node.parent_state[i])
                else:
                    node.parent_state[i] = self.lstms_parent[i](node.parent_state[i-1][0], node.parent_state[i])

        # Pass new parent state and no sibling state as we start with the first sibling
        node.sibling_state = [None for _ in range(self.params['NUM_LSTM_LAYERS'])]
    ...
    def build_tree(self, predicted_label, node_type, placeholderid_to_nameid, parent_node):
        if node_type == 'NAME':
            if predicted_label.item() < self.params['TOP_NAMES_TO_KEEP']:
                ...
                label = placeholderid_to_nameid[predicted_label.item()]
            else:
                label = f'NAME_{predicted_label.item()}'

            node = Node(label, is_reserved=False, parent=parent_node)
        else:
...
class BeamSearchNode:
    def __init__(self, node, token, logprob, node_type, parent_state, parent_state_updated, sibling_state, is_parent, has_sibling, parent_node):
        self.node = node
        self.token = token
        self.logprob = logprob
        self.processed_sibling = False
        self.processed_parent = False
        self.node_type = node_type
        self.parent_state = parent_state
        self.parent_state_updated = parent_state_updated
    ...
    def update(self, node, token, logprob, node_type, parent_state, parent_state_updated, sibling_state, is_parent, has_sibling, parent_node):
        self.node = node
        self.token = token
        self.logprob += logprob
        self.node_type = node_type
