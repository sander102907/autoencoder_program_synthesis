...
class TreeLstmDecoderComplete(nn.Module):
    ...
    def forward(self, z, target=None, idx_to_label=None):
        ...
        if target is not None:   
            ...
            individual_losses = {}
            accuracies = {}
            loss_types = list(self.embedding_layers.keys()) + ['PARENT', 'SIBLING']
    ...
    def decode_train(self, iteration, z, h_p, c_p, h_s, c_s, target, individual_losses, accuracies):
        ...
        for sibling_index in range(largest_num_siblings):
            ...
            accuracies['PARENT'] += sum((self.sigmoid(self.depth_pred(h_pred)) >= 0.5) == (is_parent == 1))
            accuracies['SIBLING'] += sum((self.sigmoid(self.width_pred(h_pred)) >= 0.5) == (has_sibling == 1))
    ...
    def decode_eval(self, parent_state, sibling_state, idx_to_label, parent_node=None, iteration=0):        
        ...
        if has_sibling:  
            if is_parent or not self.params['INDIV_LAYERS_VOCABS']:
                ...
                # Calculate next hidden sibling state
                sibling_state = self.leaf_lstms_sibling[node_type](emb_label, sibling_state)
