...
class TreeLstmDecoderComplete(nn.Module):
        ...
        for k, embedding_layer in embedding_layers.items():
            ...
            if k == 'NAME':
                ...
                self.label_losses[k] = nn.AdaptiveLogSoftmaxWithLoss(
                    params['LATENT_DIM'], params[f'{k}_VOCAB_SIZE'], cutoffs=[10, 100, 1000], div_value=2.0)
    ...
    def decode_train(self, iteration, z, h_p, c_p, h_s, c_s, target, individual_losses, accuracies):
        ...
        if iteration == 0:
            ...
            h_sibling, c_sibling = self.lstm_sibling(emb_label, (h_parent, c_parent))

            # Update the hidden and cell values matrices
            h_p[node_order == 0] = h_parent
            c_p[node_order == 0] = c_parent

            h_s[node_order == 0] = h_sibling
            c_s[node_order == 0] = c_sibling
    ...
    def decode_eval(self, parent_state, sibling_state, idx_to_label, label_to_idx, placeholderid_to_nameid, parent_node=None):
        ...
        if parent_node is None:
            ...
            if node_type == 'NAME':
                # if predicted_label.item() in placeholderid_to_nameid:
